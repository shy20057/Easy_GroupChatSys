...
public class UserInfoBeautyServiceImpl implements UserInfoBeautyService {
	...
	@Resource
	private UserMapper<User,UserInfoBeauty> userMapper;
	 ...
	 */
	@Override
	public UserInfoBeauty getUserInfoBeautyById(Long id) {
	 ...
	 */
	@Override
	public Integer updateUserInfoBeautyById(UserInfoBeauty bean, Long id) {
	 ...
	 */
	@Override
	public Integer deleteUserInfoBeautyById(Long id) {
	...
	public void saveBeautyAccount(UserInfoBeauty userInfoBeauty) {
		if(userInfoBeauty.getId() != null){
			UserInfoBeauty dbInfo = this.userInfoBeautyMapper.selectByUserId(userInfoBeauty.getUserId());
		...
		// 新增的时候判断邮箱是否存在
		if(userInfoBeauty.getUserId() == null && dbInfo != null){
			throw new BusinessException("靓号邮箱已经存在");
		}

		// 修改的时候判断邮箱是否存在
		if(userInfoBeauty.getId()!=null && dbInfo != null && dbInfo.getId() != null && !dbInfo.getId().equals(userInfoBeauty.getId())){
			throw new BusinessException("靓号邮箱已经存在");
		}
		...
		// 判断邮箱是否已经注册
		User user = this.userMapper.selectByEmail(userInfoBeauty.getEmail());
		if(user != null){
			throw new BusinessException("靓号邮箱已经注册");
		}
		user = this.userMapper.selectByUserId(userInfoBeauty.getUserId());
		if(user != null){
		...
		if(userInfoBeauty.getId() != null){
			...
			userInfoBeauty.setStatus(BeautyAccountStatusEnum.NO_USED.getStatus()); // 默认未使用
